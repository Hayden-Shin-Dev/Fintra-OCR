...
def _gold_fields(case_dir: Path, manifest: dict[str, Any], gold_source: str, gold_root: Path | None = None) -> list[dict[str, Any]]:
    ...
    if gold_source in {"semantic-v3", "semantic-v3.1", "semantic-v3.2", "semantic-v4"}:
        default_root = {"semantic-v3": "semantic-v3", "semantic-v3.1": "semantic-v3.1", "semantic-v3.2": "semantic-v3.2", "semantic-v4": "semantic-v4"}[gold_source]
...
def evaluate(cases_root: Path, output_dir: Path, strategy: str = "active", gold_source: str = "legacy", gold_root: Path | None = None) -> dict[str, Any]:
    ...
    result = {
        ...
        "gold_validity": (
            "SEMANTIC_V4_MVP_TYPED_RELATIVE_GOLD; independently built from AI-Hub word annotations and image layout without reading predictions"
            if gold_source == "semantic-v4" else
    ...
    for status, status_rows in examples.items():
        ...
        lines.append("")
    caveat = ("The semantic-v4 Gold is an MVP development benchmark: available fields are prediction-blind and image/TL-audited; unsupported families remain ambiguous_gt."
              if gold_source == "semantic-v4" else
              "The semantic gold is a reproducible typed/relative-zone annotation projection; semantic-v3 is prediction-blind and stored separately from frozen semantic-v2. It still requires human visual sign-off before being treated as a production accuracy claim."
              if gold_source in {"semantic-v2", "semantic-v3", "semantic-v3.1", "semantic-v3.2"} else
