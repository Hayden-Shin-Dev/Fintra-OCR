"""Standalone header-relative item table extraction for production.
...
def numeric_value(text):
    return text.strip() if re.fullmatch(r'\s*(?:(?:USD|EUR|GBP|JPY|CNY|KRW|US\$)\s*)?[$¥₩]?\s*[+-]?\d[\d,.]*(?:\s*(?:KG|KGS|G|PKG|PKGS|PCS|BOX|CTNS|CTN|EA))?\s*', text, re.I) else None


_DESCRIPTION = re.compile(r"\b(?:DESCRIPTION|GOODS\s+DESCRIPTION|DESCRIPTION\s+OF\s+GOODS|COMMODITY)\b", re.I)
_QUANTITY = re.compile(r"(?:\bQ(?:['’]?TY|UANTITY)\b|UNIT\s+QUANTITY|QUANTITY\s*/\s*UNIT|QTY\s*/\s*UNIT|QUANTITY\s+OR)", re.I)
