...
class MotionBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.0):
        ...
        img = cv2.imdecode(
            np.fromstring(img.make_blob(), np.uint8), cv2.IMREAD_UNCHANGED
        )
...
class GlassBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.0):
        ...
        img = np.uint8(
            gaussian(np.array(img) / 255.0, sigma=c[0], multichannel=True) * 255
        )
        ...
        img = np.clip(gaussian(img / 255.0, sigma=c[0], multichannel=True), 0, 1) * 255
...
class ZoomBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.0):
        ...
        for zoom_factor in c:
            ...
            out += (np.array(zoom_img) / 255.0).astype(np.float32)
