...
class Shrink:
    ...
    def __call__(self, img, mag=-1, prob=1.0):
        ...
        srcpt.append([P, P])
        srcpt.append([P, H - P])
        x = np.random.uniform(frac - 0.1, frac) * W_33
        y = np.random.uniform(frac - 0.1, frac) * H_50
...
class Rotate:
    ...
    def __call__(self, img, iscurve=False, mag=-1, prob=1.0):
        if np.random.uniform(0, 1) > prob:
            return img
...
class Perspective:
    ...
    def __call__(self, img, mag=-1, prob=1.0):
        ...
        high = 1 - low
        if np.random.uniform(0, 1) > 0.5:
            toprightY = np.random.uniform(low, low + 0.1) * H
            bottomrightY = np.random.uniform(high - 0.1, high) * H
            dest = np.float32([[0, 0], [W, toprightY], [0, H], [W, bottomrightY]])
        else:
            topleftY = np.random.uniform(low, low + 0.1) * H
            bottomleftY = np.random.uniform(high - 0.1, high) * H
...
class TranslateXAbs:
    ...
    def __call__(self, img, val=0, prob=1.0):
        if np.random.uniform(0, 1) > prob:
            return img
